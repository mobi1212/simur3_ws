...
class URGripper:
    def __init__(self, topic=None):
        topic = topic or cfg("gripper_topic")
        self.topic = topic
        self.pub = rospy.Publisher(self.topic, String, queue_size=1)
        rospy.sleep(0.1)

    def _topic_ready(self):
        try:
            topics = rospy.get_published_topics()
        except Exception:
            topics = []
        return any(t[0] == self.topic for t in topics)
...
def cartesian_move(arm, waypoints):
    ...
    plan, fraction = arm.compute_cartesian_path(waypoints, eef_step, 0.0)  # jump_threshold=0.0
    if fraction < 0.99:
